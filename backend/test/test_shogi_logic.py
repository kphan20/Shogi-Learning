...
class TestShogiLogicMethods(unittest.TestCase):
    def setUp(self):
    ...
    def test_get_moves(self):
        ...
        self.assertEqual(
            len(result),
            30,
            "Number of moves in starting position for black should be 30",
        )
        ...
        self.assertEqual(
            len(result),
            30,
            "Number of moves in starting position for white should be 30",
        )

        # testing pawn drops on empty board
        result = get_moves(self.empty_board, BLACK, {PAWN_ID: 1})
        self.assertEqual(
            len(result), 72, "Number of pawn drops on an empty board should be 72"
        )

        # first under check test (king and rook on board)
        result = get_moves(self.check, BLACK, {})
        self.assertEqual(len(result), 2, "Should only be two moves in this position.")

        # second under check test (starting position with rook/king)
        result = get_moves(self.check2, BLACK, {})
        self.assertEqual(len(result), 5, "Should only be five moves in this position.")

        # tests pawn force promotion
        result = get_moves(self.pawn_prom, BLACK, {})
        # print_moves(result)
        self.assertEqual(len(result), 1, "Pawn should be force promoted.")
